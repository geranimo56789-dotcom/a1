# Fast Batch Logo Downloader
param([string]$OutputDir = "football_logos", [int]$BatchSize = 5)

if (!(Test-Path $OutputDir)) { New-Item -ItemType Directory -Path $OutputDir -Force | Out-Null }

# Essential teams with verified URLs
$essentialTeams = @{
...
# League mapping for organization
$teamLeagues = @{
    "Eintracht Frankfurt" = "Germany"; "VfL Wolfsburg" = "Germany"; "SC Freiburg" = "Germany"; "Werder Bremen" = "Germany"; "Mainz 05" = "Germany"
    "Lens" = "France"; "Marseille" = "France"; "Rennes" = "France"; "Monaco" = "France"; "Lille" = "France"; "Lyon" = "France"; "Nice" = "France"
    "Villarreal" = "Spain"; "Osasuna" = "Spain"
    "Aston Villa" = "England"; "Crystal Palace" = "England"; "Fulham" = "England"
    "Juventus" = "Italy"; "Atalanta" = "Italy"; "Fiorentina" = "Italy"; "Bologna" = "Italy"; "Torino" = "Italy"
    "Sporting CP" = "Portugal"; "Braga" = "Portugal"
    "Corinthians" = "Brazil"; "Santos" = "Brazil"; "Fluminense" = "Brazil"
    "Rangers" = "Scotland"; "Hearts" = "Scotland"; "Aberdeen" = "Scotland"
    "Galatasaray" = "Turkey"; "Fenerbahce" = "Turkey"; "Besiktas" = "Turkey"
    "Al Hilal" = "Saudi_Arabia"; "Al Nassr" = "Saudi_Arabia"
    "Young Boys" = "Switzerland"; "Basel" = "Switzerland"
    "Olympiacos" = "Greece"; "PAOK" = "Greece"
    "LA Galaxy" = "USA"; "LAFC" = "USA"; "Seattle Sounders" = "USA"; "Inter Miami" = "USA"
}
...
function Quick-Download {
    ...
    if (Test-Path $filepath) {
        Write-Host "✓ $TeamName" -ForegroundColor Gray
        return $true
    }
    
    try {
        Invoke-WebRequest -Uri $Url -OutFile $filepath -TimeoutSec 5 -ErrorAction Stop
        Write-Host "✅ $TeamName" -ForegroundColor Green
        return $true
    } catch {
        Write-Host "❌ $TeamName" -ForegroundColor Red
        return $false
    }
}

$successful = 0
$total = $essentialTeams.Count
$processed = 0

Write-Host "🚀 Quick Batch Logo Downloader" -ForegroundColor Cyan
Write-Host "Processing $total missing teams..." -ForegroundColor Yellow

foreach ($team in $essentialTeams.Keys) {
    $processed++
    Write-Host "[$processed/$total] " -NoNewline
    
    $league = $teamLeagues[$team]
    if (Quick-Download $team $essentialTeams[$team] $league) {
        $successful++
    }
    
    # Small pause every batch
...
# Quick summary
Write-Host "`n📊 Batch Results: $successful/$total" -ForegroundColor Cyan

# Count all files
$allFiles = Get-ChildItem $OutputDir -Recurse -Filter "*.png" | Measure-Object
Write-Host "Total logos now: $($allFiles.Count)" -ForegroundColor Green
